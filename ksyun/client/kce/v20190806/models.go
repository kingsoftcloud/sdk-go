...
type ForceRemoveClusterInstanceRequest struct {
    *ksyunhttp.BaseRequest
    ClusterId *string `json:"ClusterId,omitempty" name:"ClusterId"`
    InstanceId []*string `json:"InstanceId,omitempty" name:"InstanceId"`
}

func (r *ForceRemoveClusterInstanceRequest) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}

func (r *ForceRemoveClusterInstanceRequest) FromJsonString(s string) error {
    f := make(map[string]interface{})
    if err := json.Unmarshal([]byte(s), &f); err != nil {
        return err
    }
    if len(f) > 0 {
        return errors.NewKsyunSDKError("ClientError.BuildRequestError", "ForceRemoveClusterInstanceRequest has unknown keys!", "")
    }
    return json.Unmarshal([]byte(s), &r)
}

type ForceRemoveClusterInstanceResponse struct {
    *ksyunhttp.BaseResponse
    RequestId *string `json:"RequestId" name:"RequestId"`
	InstanceSet []struct {
		InstanceId *string `json:"InstanceId"`
...
func (r *ForceRemoveClusterInstanceResponse) ToJsonString() string {
    b, _ := json.Marshal(r)
    return string(b)
}

func (r *ForceRemoveClusterInstanceResponse) FromJsonString(s string) error {
    return json.Unmarshal([]byte(s), &r)
}
